...
Phase handoff: Batch 0001 prose and manuscript state are complete, and `reviews/volume-01/batch-0001.md` records the batch audit. The next writer should begin with `workspace/volume-01/batch-0002/PROMPT.md`, which now lists the beats already played in Chapters 1–10 so they are advanced rather than repeated. Controller-owned files were not edited: `state/phase-ledger.json` still shows `phase-002-batch-plan` as planned, and the `.done` markers for `workspace/phase-002-batch-plan/` and `workspace/volume-01/batch-0001/` must be written by the controller. Until they exist, phase selection will land on a completed batch, so both prompts carry completion banners that forbid rewriting finished chapters.
